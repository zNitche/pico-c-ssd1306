...
def get_args():
    ...
    argument_parser.add_argument(
        "--output-file-name", type=str, help="output bitmap file name", required=False)
...
def dump_to_file(img_width: int, img_height: int, output_buffer: list[str], output_path: str,
                 dump_to_h: bool, name_override: str | None = None):

    output_filename = str(datetime.timestamp(datetime.now())) if not name_override else name_override
